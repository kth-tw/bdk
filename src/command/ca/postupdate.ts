...
export const command = 'postupdate'

export const desc = 'Postupdate CA certificates and restart.'
...
export const builder = (yargs: Argv) => {
  return yargs
    .example('bdk ca postupdate --interactive', 'Cathay BDK 互動式問答')
    .example('bdk ca postupdate --ca-name ica', '關閉名稱為 ica 的 CA 機器')
...
export const handler = async (argv: CaDownParams) => {
  ...
  if (argv.interactive) {
    ...
    await ca.postupdate({ caName: info.caName })
  } else {
    await ca.postupdate({ caName: argv.caName })
  }
}
